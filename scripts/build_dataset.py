...
- 全{food_count}食品の `name` を事前に埋め込んだベクトルを `/embeddings/ruri-v3-30m.f32`
  （float32・行優先・shape `[{food_count}, 256]`。行の並びは同ディレクトリの `ruri-v3-30m.codes.json` の食品番号列と対応）として公開している。
